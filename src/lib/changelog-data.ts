...
export const LATEST_VERSION = "v0.46.52";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "主页整体调亮、对比加强：页背景/卡片/文字全部提亮一档（深蓝黑 #0E1424 + 近白文字 17:1 对比），书封/书架同步亮化并补 color-mix 兜底（老浏览器不再一团黑）",
  "顶部 UI 栏下移为悬浮式（sticky top-2 + 圆角浮起 + 背景），不再贴死顶部，系统提示文字完整可见",
  "纸舟星海换现代名船：黑珍珠号（黑色三桅黑帆金饰）、复仇女王号（黑胡子旗舰+骷髅）、飞翔的荷兰人（幽灵船青灰破帆幽绿光）、航空母舰（宽平甲板+舰岛+斜角甲板+甲板舰载机「子舰=作品」）、驱逐舰（舰桥+主炮+导弹架）、核潜艇（艇身+围壳+潜望镜）；题材→名船映射重排，交互（悬浮书名/点击确认/旋转缩放/掉落）全保留",
  "精简理念文案：删除「设计说明」四卡区块，区块说明压成一句；预览文件重写同步（现代名船+漫画海+确认+悬浮书名）；tsc 零错误 + dev 200 自查通过",
];
