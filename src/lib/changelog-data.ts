...
export const LATEST_VERSION = "v3.1.55";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "v3.1.55 数据库目录不再可能被误传上网 + 记一次失败的架构尝试（DATA-GITIGNORE）：①真问题——.gitignore 只写了 `*.db`，这只挡住数据库本体，挡不住同目录下的其它文件（比如我给数据库做备份时生成的 novelforge.db.bak-20260830 这类带时间戳的备份文件）；而 data/ 目录里存着你在设置页填的 AI 钥匙、以及全部小说正文和设定，只要这类文件被 commit 一次，就是「钥匙泄露 + 全部稿件外泄」的既成事故，且 Git 历史里删不干净。现在改成整个 data/ 目录一律不入库（顺手补上 db-wal/db-shm/sqlite 等后缀），从「记住别传某个后缀」变成「这个目录根本进不来」，堵死这一类手滑。②本轮还做了一次架构尝试并有结论——想让数据库直接支持「结构化字段」（不用再手工打包解包），实测发现：新版本的数据库工具已经支持 Json 类型，但对「默认值」的处理有 bug（会生成数据库不认识的裸方括号语法），强行推进会牵动大量写入处代码、风险不可控，已完整回滚（数据库完整性校验通过、31 张表 48 行数据与改动前逐条一致、体积也已压缩复原）。③结论落盘：这条路暂时走不通， serialize 层的 162 个类型问题改走「业务侧逐步适配」的保守路线，不影响日常使用（开发模式不受类型问题影响，项目实测首页、项目接口、健康检查全部正常）。个人 IP 仍归瑞宝宝。",
...
export const CHANGELOG_USER_BRIEF = [
  "v3.1.55 堵上一个「可能把你的 AI 钥匙和全部稿件传上网」的口子：之前项目里有个名单写着「不许上传后缀是 .db 的文件」，用来保护存着你 AI 钥匙和全部小说的数据库。但这份名单只挡那一种后缀——同目录下的其它文件管不着，比如哪天给数据库做个备份、文件名叫 xxx.db.bak-20260830，它就绕过去了。这种事一旦发生就很难挽回：钥匙和稿子都在别人手里了，而且网上的提交历史删不干净。现在改成「整个存放数据库的文件夹一律不许上传」，不管里面放什么文件都进不来，从「记住别传某一种文件」变成「这个文件夹根本传不上去」。另外这轮还试了一次底层改造（想让数据库直接存结构化数据、省掉手工打包解包这一步），实测发现新版本的工具有个 bug 会生成数据库看不懂的语法，硬推风险太大，已经完整撤回来了——数据一条没少（校验过：31 张表、48 行，跟改之前一模一样），项目实测首页、项目列表、健康检查都正常。个人 IP 仍归瑞宝宝。",
...
export const VERSIONS: VersionEntry[] = [
  {
    ...
    sections: [
      ...
      {
        label: "🧪 架构尝试：SQLite 原生 Json 化（已回滚，结论留档）",
        items: [
          ...
          "阻断项：Prisma 对 Json 字段的 @default 会生成非法 SQL —— DEFAULT []（裸方括号，SQLite 不认），db push 报 unrecognized token: \"{\"",
          "规避需去掉全部 Json 字段默认值，牵动大量 create 调用点，风险不可控，故完整回滚",
          "回滚验证：schema / prisma.ts / 数据库三方恢复一致；integrity_check ok；31 张表 48 行数据与改动前逐条一致；VACUUM 后体积复原",
          "功能验证：dev 模式首页 HTTP 200、/api/projects 返回正常（toneKeywords 数组反序列化正确）、/api/health db.ok 与 llm.ok 均为 true",
        ],
      },
      {
        label: "📌 结论与后续",
        items: [
          "162 个类型错误改走保守路线（业务侧逐步接入 safeJoin / safeSplit），不再动数据层架构",
          "该错误不影响日常使用：dev 模式不阻断类型错误，仅 npm run build 与 CI 的 Type check 受影响",
          "待办：CI 转绿、README 快速开始、首次运行体验（面向 clone 后自行部署的开源用户）",
        ],
      },
    ],
  },
