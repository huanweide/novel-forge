...
export const CHANGELOG_BRIEF = [
  "v1.6.0 自动填表补全角色关系：章节填表自动检测角色间关系并写入角色卡（新卡带关系、已存在卡补关系合并去重）",
  "批量写作升级两段流：先生成 N 章章纲（可逐章编辑/勾选）→ 确认后后台生成正文，全流程可关窗口",
  "单章生成确认内置章纲循环：先生成章纲（可编辑/修复）再确认生成正文；缝合怪新增节奏调控（快/均衡/慢热）",
  "故事线全屏总览弹窗（主线+支线完整过程+打勾）；删除冗余时间线视图；tsc 零错误 + 217 测试全绿",
];
