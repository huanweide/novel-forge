...
export const LATEST_VERSION = "v0.46.47";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "作品区改造为「书架」：ProjectCard 升级为 3D 立体书（题材色封面+左侧深色书脊+底部书页厚度），网格容器变书架板（一排排整齐立书），hover/焦点有「从书架抽出」的突出效果（上浮+发光+题材色辉光），暗/浅主题双适配",
  "纸舟星海材质修饰：船体宣纸纹、木料木纹、帆帆布纹、篷竹篾纹——四种 Canvas 纹理客户端惰性构建（SSR 安全），船从「平涂色块」变成有质感的墨色一笔",
  "水效果与动感加强：海浪波幅加大（0.19/0.13）+ 浪花高光 foam + 边缘光增强，水更明显可辨；船起伏摇曳幅度加大（rotation ±0.10/±0.07）；系统开启「减弱动态」时船仍随海起伏（仅相机静止不漂移）",
  "点击聚焦增强：聚焦船放大 1.15→1.32、相机推进 lerp 加快（0.06→0.085），点船拉近望「下一章」的反馈更明显；预览文件同步重写（去花哨+新材质+书架示意）",
];
