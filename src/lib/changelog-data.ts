...
export const LATEST_VERSION = "v0.30.0";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "🎭 分阶段人设求值生效：<if cell=\"表/行/列 <= 阈值\"> 语法现在真正按表格数值定档——角色随剧情进展自动换性格阶段",
  "🧠 剧情推进=人设进化：套用「好感度·分阶段人设模板」后，写章节会自动注入当前好感度对应的苏苏人设阶段，不再把语法标签丢给 AI",
  "🛡 安全降级：属性表缺失/数值非法时返回全阶段参考文本，绝不会误判为某一阶段或暴露原始标签",
  "✅ 单元 + 端到端双层验证：好感度 5/25/50/80 精准命中阶段一~四，写章节 recall 事件携带的是已求值人设",
];
