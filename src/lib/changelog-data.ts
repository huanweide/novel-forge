...
export const LATEST_VERSION = "v0.46.58";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "名字高亮优化：①标题/章节头部（章头）不再显示高亮标记，只在正文高亮；②新增常用词停用表（她/什么/现在等高频词即使被注册成实体名也不高亮）——「世界卡」「角色卡」等完整词条名正常高亮，误判大幅减少",
  "Agent 助手「墨灵」全面升级：起名+渐变高亮身份；头部新增「墨灵能做什么」教学（角色/世界书/大纲/伏笔/关系网/分析六类能力）；设置页新增 Agent 模式开关——默认「可操作」（能查询并修改项目数据），切「只读」后仅查信息、不写数据、响应更快（后端已拦截写工具）",
  "备份包升级：导出前弹窗勾选「保留哪些设定」（角色/词条/章节/分支/剧情线/文风/表格/正则），未勾选不导出；导入备份包同样弹窗选择保留项再导入为新项目——顺带修复备份 API 因中文文件名导致的 500 隐藏 bug",
  "导入合并：写作页「导入设定」「导入书稿」统一进同一个导入向导（书稿=分章+抽三卡 / 设定=仅抽三卡不建章节），三卡全程可交互编辑；摘要按钮新增确认（说明范围=仅本章、产出=摘要+关键事件+角色状态+事件重要度）",
  "正则规则：项目配置中心新增「从预设添加」——从创意工坊已有正则预设一键添加（无需手写名字/pattern）；记忆衰减在设置页新增说明（本地手动/外部 cron 自动，S永久/A30章/B15章/C5章）；游戏模式新增首次教程+已有正文带入（游戏从本章已有内容后续接，含原字数）",
];
