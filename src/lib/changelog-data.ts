...
export const LATEST_VERSION = "v3.1.88";
...
export const CHANGELOG_BRIEF = [
  ...
  "v3.1.74 伏笔搜索（FS-SEARCH · 几十条线索里一秒捞出要看的那条）：①伏笔追踪面板顶部新增紧凑搜索框（放大镜 + 输入框 + 清空 X），线索攒到几十上百条后不用再一个分组一个分组地翻。②模糊匹配范围比大纲/世界书更广——线索描述、AI 给你的「后续发展思路」、来源（搜「推断」「用户」「大纲」）、优先级（搜「高」「中」「低」）、埋设章节号 / 应回收章节号（搜「第12章」）全能命中；中英文都行，不用记精确措辞。③命中保留原分组结构（待收尾 / 部分收 / 已收 / 已废弃），空的组自动隐藏，不破坏你对线索状态的既有认知。④实时计数「匹配 X / Y 条线索」；无匹配走独立分支「没有匹配「{关键词}」的线索」+「清空搜索」，与「暂无未收尾线索」真空态分开，避免误以为线索丢了。⑤零数据层改动：纯前端 useState + useMemo，不动 schema / API / 存储；只改 ForeshadowingPanel.tsx 单文件 + 新增 7 条单测；无障碍：input 与按钮均有 aria-label。⑥匹配逻辑抽成模块级纯函数 matchForeshadowItem 并配 7 条单元测试（空关键词不过滤 / 中英文来源 / 章节号 / 字段缺失不炸），这是本轮唯一有逻辑的新代码，测试全覆盖。个人 IP 仍归瑞宝宝。",
  "v3.1.73 世界书搜索（WORLD-SEARCH · 烂笔头设定多了照样一秒定位）：①世界书每个板块内新增紧凑搜索框（放大镜图标 + 输入框 + 清空 X），叠加在原有的板块粗筛（地理/种族/势力/历史/设定/其他）之上——先选板块再在板块内搜，两级过滤。②模糊匹配词条标题 / 别名 keys（数组自动拼接）/ 正文前 200 字符，中文直接搜、不用记精确名字；命中即过滤、零延迟反馈。③实时计数「匹配 X / Y 个词条」，词条多少一眼可见。④无匹配走独立分支「没有匹配「{关键词}」的地理词条」+「清空搜索」按钮——与「暂无地理设定」这个真空态分开，避免你误以为设定丢了。⑤零数据层改动：纯前端 useState + useMemo，不动 schema / API / 存储；只改 WorldPanel.tsx 单文件；无障碍：input 与按钮均有 aria-label。⑥顺带修正 v3.1.72 遗漏的一处版本号（changelog-data.ts 的 LATEST_VERSION 当时没跟着升，停在 v3.1.71），现与 package.json / CHANGELOG.md 三处对齐 v3.1.73。个人 IP 仍归瑞宝宝。",
  "v3.1.72 大纲搜索（OUTLINE-SEARCH · 长篇高频刚需）：①写作台左栏大纲顶部新增紧凑搜索框（放大镜图标 + 输入框 + 清空按钮），输入即过滤、零延迟反馈；模糊匹配章节标题 / 正文前 200 字符 / 状态文字（搜「草稿」「已确认」等中文也行），命中节点保留祖先链（搜「第 5 章」也要看到它所属的卷不被隐藏），搜索框下方实时显示「匹配 X / Y 个节点（含 Z 个祖先卷/分卷）」。②搜索无结果独立分支：「没有匹配「{关键词}」的章节」+「清空搜索」按钮，与「没有章节」空状态分开，避免用户误以为项目空了。③零数据层改动——纯前端 useState + useMemo 计算，不动 schema / API / 存储；只改 OutlineTree.tsx 单文件；性能可控、useMemo 缓存命中、状态映射表按需定义；无障碍：input 与按钮均有 aria-label。④质量门禁——类型检查 0 错误、vitest 1306 全绿、生产构建通过。个人 IP 仍归瑞宝宝。",
  ...
  "v3.1.61 首页新增「灵感火花」创意启发板块（SPARK）：①不知道写什么时，点一下「抽一张灵感牌」——系统从题材×开局×张力×反转四个维度随机组合出一段完整灵感（如「仙侠｜一觉醒来昨天的记录都被抹去…核心张力：记忆与遗忘；一句话反转：而真相是这一切都是你亲手设计的」），纯前端本地生成、不联网不调 AI 不花钱，契合本地优先定位。②「换一张」按钮即时重抽（骰子图标旋转动画），「用这个开局」带 genre+inspiration 参数跳进探讨模式，AI 会接着这枚火花往下聊。③板块沿用首页「虚空玻璃」设计语言（题材色光晕+elevation，悬浮微浮起发光），浅色/深色主题自适应，复用现有 Lucide 图标体系、禁 emoji。④工程——新增 InspirationSpark 组件与配套 CSS、扩展 /explore 消费 inspiration 参数给出针对性开场白；质量门禁类型检查 0 错误、1289 条测试全绿、生产构建通过。个人 IP 仍归瑞宝宝。",
  ...
  "v3.1.54 上线体检 + 4 个 P0 修复：CI 连红 7 天的真凶找到了（BUILD-UNBLOCK）：①根因——prisma-serialize 的 applySerialization 返回 any，把整个数据层类型推导全部擦除，于是 126 处回调参数退化为隐式 any（TS7006）、CI 的 Type check 硬门禁从 v3.1.49 起一路红到 v3.1.53（实证 main 分支连续 failure、卡点均为 analyze-chapter 路由 TS7006），而 build 步骤因排在 tsc 之后从未真正跑过；改为泛型签名后 TS7006 归零，同时掀出 162 个原被 any 掩盖的真实类型不匹配。②Date 被序列化层摧毁为空对象——序列化与反序列化对任何 object 都用 Object.keys 递归重建，而 Date/Buffer/Decimal 的自有可枚举键为空、重建结果恒为空对象；后果是写入时间字段变空对象触发 Prisma 报「Expected DateTime, provided Object」（会话测试因此挂），读取创建/更新时间也被抹平、项目接口实测返回空对象、首页卡片时间显示 Invalid Date；新增 isOpaque 精确黑名单（Date/RegExp/Map/Set/TypedArray/ArrayBuffer/带 toFixed 的 Decimal）原样透传，实测时间字段恢复 ISO、会话测试 4/4 转绿。③safeJoin 漏 import——v3.1.50 的 20+ 处 join 链修复只改了调用点，6 个文件 8 处调用了 safeJoin 却没补 import，属运行时 ReferenceError 级隐患，已幂等补齐。④设置页保存自相矛盾——保存函数的拦截条件漏了「已存在 Key」兜底（同文件另外 4 处均已正确带上），与界面承诺的「已保存 · 留空则不修改」直接冲突：已配置用户出于安全不回显 Key、输入框天然为空，于是只想改模型或接口地址都会被拦下、必须重粘整串 Key；连带修掉两个会误清 Key 的隐患（留空时传空串会覆盖库中 Key、保存后把已保存标记清掉）；端到端三段验证：写入 Key → 留空保存 → 模型已改且 Key 保留，对照组传空串确实清空。⑤门禁实测——单测 1226 通过/0 失败/6 跳过；绕过类型检查跑生产构建得 EXIT=0（编译 26.7 秒、91 个静态页、140 条路由全过），证明除类型错误外无其它构建阻断；生产产物冒烟 10/10 路由 HTTP 200、生产模式时间字段同样正常。⑥体检结论——13 个页面逐页审完（均分 8.2/10，最低是工坊页与表格页 7/10，因加载失败静默或缺 try-catch 导致永久转圈），接口层 128 条路由零鉴权、限流仅覆盖 8 条、烧钱类生成路由全裸，故当前可自托管内网使用、不可公开部署；剩余 162 个类型错误本质是序列化层「运行时改了形状、类型没跟上」的契约缺口（读侧报属性不存在、写侧报数组不可赋给字符串），非运行时 bug，需专项收敛后 CI 才能转绿。个人 IP 仍归瑞宝宝。",
  ...
  "v3.1.25 自检 banner AI 项黑话收敛·配置错误可读化（UI 设计师·系统自检横幅·体验修复）：①真缺陷——顶部「系统自检发现 N 项需处理」横幅的 AI 配置项，本该明确告诉本地部署者「AI 没配好、去设置页填 Key」，却因错误分类器 classifyError 默认分支把我们自己写好的清晰中文提示兜底成「服务器内部错误，请查看日志」，部署者看到黑话无从下手；本轮新增「配置类友好错误」透传分支，当 err.message 已含 未配置/未填写/请在设置/填入/选择提供商/选择模型/Base URL/API Key/API 地址/本地推理需/无法解析 等中文配置关键词时直接原样透传、不再黑话化，横幅 AI 项现在直接显示「LLM API Key 未配置——请在设置页面填入 Key，或在 .env 中设置 LLM_API_KEY」并配「去设置页填 Key →」链接；②零风险——透传分支只命中我们自己写的中文配置提示、绝不命中原始堆栈/SQL 片段，真实内部错误仍走 500 兜底不泄露内部信息，classifyError 补 3 例自动化测试锁死透传/不透传契约；③质量门禁——零生产逻辑删除、纯错误提示文案路由修正、零接口/LLM 变化；tsc 0 错误（2 个需真实 Postgres 的 DB 集成测试在沙箱无库环境下连接失败，与本次改动无关）；vitest 全量单测全绿；六处版本文件对齐 v3.1.25；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.24 游戏白天主题文本对比度收口（UI 设计师·虚空玻璃体系·无障碍·WCAG 1.4.3）：①游戏白天主题对比度收口——前几轮无障碍体检（v3.1.23 键盘焦点环/焦点色加浓）只覆盖工作区三主题（dark/light/azure），游戏模式白天主题 [data-game-theme=day] 从未纳入；该主题 --nv-text-muted 原值 #6A6F7C 在浅色页面底 void(#EEF2F7)仅 4.47:1、在浮起面 surface-3 仅 3.65:1（<4.5 AA 文本对比门槛），本轮深化为 #5C606B——void 5.59/白底 6.29/surface-3(叠页面底)4.56 全 ≥4.5 达 AA，且仍浅于 tertiary #565B68 保层级；②补齐缺失令牌防回退崩坏——游戏白天主题此前缺 --nv-text-muted-on-surface-3 专用令牌，共享组件（AIChatHeader/MonitorPanel/MemoryDecayDialog/dissect）在该作用域引用此令牌会回退到深色主题浅灰值 #96948B 致浅底对比崩坏，本轮补 --nv-text-muted-on-surface-3: #5C606B 与 base 同值保持四主题一致；③对症不画蛇添足——用全矩阵对比度体检脚本确认四主题中仅游戏白天确凿 FAIL、base 已全员达标故不动；grep 全文件零裸 hex/离谱色、本轮零新增色；④质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误（2 个需真实 Postgres 的 DB 集成测试在沙箱无库环境下连接失败，与本次改动无关）；vitest 全部非 DB 单测 1190 例全绿；六处版本文件对齐 v3.1.24；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.23 键盘焦点可见性修复（UI 设计师·虚空玻璃体系·无障碍·WCAG 2.4.7 / 1.4.11）：①键盘焦点环补位——新增全局 :where(...) 选择器覆盖 role=button/tab/option/radio/menuitem/tabindex 等自绘可聚焦控件，给故事线因果链节点、行内编辑块等原本键盘可达却无高亮的 div 画上焦点环，键盘用户 Tab 进去终于看得见焦点在哪；②焦点色加浓达标——button:focus-visible 与新增全局规则改实心 ring-ring（去掉半透明叠加），:root 与 .light 的 --ring/--sidebar-ring 改实心、浅色加深至 oklch(0.52)，深底靛蓝/浅底深靛蓝/苍青青/暗金四套焦点环均 ≥3:1 达 WCAG 非文本对比，.dark 暗金与苍青青环原本已达标保留不动；:where() 零优先级不覆盖组件级显式 focus 样式，仅键盘聚焦触发、鼠标点击不显示；③质量门禁——本次仅 CSS 令牌与全局焦点规则变更、零生产逻辑删除、零接口/LLM 变化，tsc 0 错误（2 个需真实 Postgres 的 DB 集成测试在沙箱无库环境下连接失败，与本次改动无关），vitest 全部非 DB 单测 1190 例全绿；六处版本文件对齐 v3.1.23；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.22 大纲树节点行项微交互柔化（UI 设计师·虚空玻璃体系·组件级精修·大纲栏）：①大纲树节点行项柔化——OutlineTree.tsx 大纲树主节点（NodeTreeItem，写作工作台左侧章节/卷结构导航行项）外层原 transition-shadow 仅过渡阴影、hover 的背景/文字色/左侧 inset 高亮条全是瞬间跳变；本轮升级为 transition-all duration-150（背景+文字+左侧条平滑过渡），悬停从生硬跳变变为丝滑过渡，与 v3.1.20 角色行项同范式（均为行项列表 hover 仅 transition-shadow 导致瞬间跳变的同源问题）；②对症不画蛇添足——大纲树为嵌套树形结构、节点紧密排列，浮起 -translate-y 会破坏缩进对齐与树形视觉，故只做「柔化过渡」、不加浮起（与 v3.1.20 角色行项一致），选中态（isSelected 品牌主色淡底 + inset 主色左边条）与删除按钮平滑浮现（transition-opacity）本已完善、未改动；③品牌一致性——grep 全文件零裸 hex/离谱色命中，所有颜色走 --nv-* 令牌、本轮零新增色；④质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿；六处版本文件对齐 v3.1.22；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  ...
  "v3.1.16 故事线工作台吸顶子栏去滤纸（UI 设计师·虚空玻璃体系·组件级精修）：①组件内 sticky 子标签导航栏去滤纸——StorylineWorkbench.tsx 内部 sticky 吸顶子栏（把三要素/时间轴/线索集/因果链变可切换视图）的 backdrop-blur-md 降级为 backdrop-blur-sm（无色玻璃身份），背景本就是实色 --nv-surface-2、md 模糊纯属冗余滤纸；②品牌一致性——grep 全文件零 backdrop-blur 残留（仅此 1 处 md 已降级）、零裸 hex/离谱色命中，所有颜色早已走 --nv-* 令牌、本轮零新增色；③质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.15 同基线、本版零新增测试、纯 UI/CSS）；六处版本文件对齐 v3.1.16；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.15 更新公告页视觉层次升级（UI 设计师·虚空玻璃体系·逐页精修·收官页）：①顶栏去滤纸——changelog 更新面板页 header 的 backdrop-blur-md 降级为 backdrop-blur-sm（无色玻璃身份），与回收站/探索/表格/设置/工坊一致；②整页轻淡入——最外层 min-h-screen 容器加 animate-in fade-in（轻量淡入、不位移，避免干扰时间线列表布局），贴合全站转场体系；③品牌一致性——grep 全文件零裸 #hex/离谱色命中，所有颜色早已走 --nv-* 令牌、本轮零新增色；④质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.14 同基线、本版零新增测试、纯 UI/CSS）；六处版本文件对齐 v3.1.15；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.14 游戏模式视觉层次升级（UI 设计师·虚空玻璃体系·逐页精修）：①整页轻淡入——游戏模式（沉浸式互动文本冒险 /workspace/[projectId]/game/[nodeId]）根容器 game-root 补 animate-in fade-in 轻量淡入（无滤纸可去：grep 全文件零 backdrop-blur-md+，顶栏/遮罩/pill/footer 均为合法 backdrop-blur-sm 薄玻璃；不位移，避免干扰 canvas/粒子/指针光晕等沉浸式动效），贴合全站转场体系；②品牌一致性——grep 全文件仅一处 #E4B863 品牌香槟金兜底（canvas 不吃 CSS 变量、v3.1.7 已统一去紫），零裸 hex/离谱色，所有颜色早已走 --nv-* 令牌、本轮零新增色；③质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.13 同基线、本版零新增测试、纯 UI/CSS）；六处版本文件对齐 v3.1.14；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.13 写作工作台本体视觉层次升级（UI 设计师·虚空玻璃体系·逐页精修）：①整页轻淡入——写作工作台本体（三栏写作界面：大纲栏/正文区/侧栏）根容器 h-screen 补 animate-in fade-in 轻量淡入（该页零 backdrop-blur，无滤纸可去；不位移，避免干扰三栏 flex 布局与写作光标焦点），贴合全站转场体系；②品牌一致性——grep 全文件无裸 hex/离谱色命中（仅注释内的 #124/#123 变更编号），所有颜色早已走 --nv-* 令牌、本轮零新增色；③质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.12 同基线、本版零新增测试、纯 UI/CSS）；六处版本文件对齐 v3.1.13；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.12 工作台表格页视觉层次升级（UI 设计师·虚空玻璃体系·逐页精修）：①顶栏去滤纸——tables header 的 backdrop-blur-md 降级为 backdrop-blur-sm（无色玻璃身份），与探索/回收站/设置/工坊一致；表格头 thead 原本是 sticky 吸顶无模糊、保持不动；②整页轻淡入——最外层 min-h-screen 容器加 animate-in fade-in（不位移，避免干扰虚拟滚动表格与吸顶表头布局），贴合全站转场体系；③品牌一致性——表格页所有颜色早已走 --nv-* 令牌、零裸 hex/离谱色（grep 全文件无 #hex/purple/pink/violet 命中）、本轮零新增色；④质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.11 同基线、本版零新增测试、纯 UI/CSS）；四处版本文件对齐 v3.1.12；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.11 探索工作台视觉层次升级（UI 设计师·虚空玻璃体系·逐页精修）：①顶栏去滤纸——explore header 的 backdrop-blur-md 降级为 backdrop-blur-sm（无色玻璃身份），与回收站/设置/工坊一致；左/右抽屉（BuildConfigPanel/CardBrowser 等）原本已是合法 backdrop-blur-sm 保持不动；②整页轻淡入——最外层 min-h-screen 容器加 animate-in fade-in（不位移，避免干扰聊天工作台布局），贴合全站转场体系；③品牌一致性——探索页所有颜色早已走 --nv-* 令牌、本轮零新增色，无游离离谱色；④质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.10 同基线、本版零新增测试、纯 UI/CSS）；四处版本文件对齐 v3.1.11；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.10 回收站视觉层次升级（UI 设计师·虚空玻璃体系·逐页精修）：①项目回收站补标题 header——回收站原「项目回收」区块只有一行灰字描述、没有标题，与下面「节点回收站」有图标标题不对称、辨识度低；本轮补 package 图标 + 「项目回收站」标题（与节点回收站的 file 图标 + 标题对称），两块回收区一眼分清；②顶栏去滤纸——recycle header 的 backdrop-blur-md 降级为 backdrop-blur-sm（无色玻璃身份），与设置/工坊一致；③整页入场转场——main 内容容器加 animate-in fade-in slide-in-from-bottom-2 轻量上滑淡入，贴合全站转场体系；④质量门禁——零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.9 同基线、本版零新增测试、纯 UI/CSS）；四处版本文件对齐 v3.1.10；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.9 设置页视觉层次升级（UI 设计师·虚空玻璃体系·逐页精修）：①section 图标徽章 header——设置页原先 8 个分区（外观/提供商/API Key/模型/违禁词/快捷键/记忆衰减/墨灵）都是纯文字数字标题、辨识度低、像一份表单清单；本轮加统一的 .settings-badge 语义图标徽章（圆角方块 + 主色浅底描边，取自 --nv-primary 令牌、深浅主题自适应），每个分区配 Lucide 图标（palette/cloud/key/bot/shield/zap/history/sparkles），一眼看出是哪一区，建立视觉层次而不杂乱；②记忆衰减等级色徽章——S/A/B/C 四级原先是纯文字，本轮加 .mem-tier 语义色徽章（S=香槟金/A=靛蓝/B=青/C=灰），衰减优先级直观可辨；③顶栏去滤纸——settings header 的 backdrop-blur-md 降级为 backdrop-blur-sm（无色玻璃身份），与工坊 v3.1.8 一致；④整页入场转场——内容容器加 animate-in fade-in slide-in-from-bottom-2 轻量上滑淡入，贴合全站转场体系；⑤零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；⑥质量门禁——tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.8 同基线、本版零新增测试、纯 UI/CSS）；四处版本文件对齐 v3.1.9；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.8 创意工坊预设卡片视觉升级（UI 设计师·虚空玻璃体系·逐页精修）：①预设类型标签语义分色——创意工坊 8 种预设类型（表格模板/剧情推进/文风/世界观/角色卡/世界书/正则/API 参数）原先统一是靛蓝同色文字、辨识度低，本轮给每种类型配品牌语义色 + Lucide 图标（类型→靛蓝/翠绿/香槟金/青/琥珀/金/玫瑰/靛蓝，全部取自 --nv-* 令牌、技术配置类表格+API 同归靛蓝族靠图标区分、绝不引入离谱紫粉），标签变成「图标+文字」且按类型着色，一扫即知是哪类预设；②卡片悬浮微交互升级——卡片新增 .preset-card 类，hover 时整卡上浮 3px、按该预设类型色描边发光（color-mix 类型色光晕），取代原来只有顶边一条渐变线，沉浸感与材质感显著提升；③顶栏去滤纸——header 的 backdrop-blur-md（中等毛玻璃）降级为 backdrop-blur-sm（无色玻璃身份），更清爽、回应「不要一堆滤纸效果」；④零生产逻辑删除、纯视觉/CSS 增强、零接口/LLM 变化；⑤质量门禁——tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.7 同基线、本版零新增测试、纯 UI/CSS）；四处版本文件对齐 v3.1.8；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.7 关系图谱配色品牌化收口（UI 设计师·虚空玻璃体系·收尾补刀）：①关系图谱最后一块离谱色——RelationshipGraph.tsx 的 roleColor（8 角色节点色）与 relationStroke（5 类关系连线色）原本混用靛(#818cf8)/粉(#f472b6)/紫(#a78bfa) 等离谱色，本轮全部收敛到品牌语义令牌（主角→香槟金 --nv-creative 单一身份色、反派→--nv-danger 玫瑰、正角→--nv-success 翠绿、师长→--nv-warning 琥珀、情感线→--nv-primary 靛蓝品牌主色、调剂→--nv-info 青、催化剂→冷调石板蓝 #7C8DB0 数据可视化专用、背景→--nv-text-tertiary 灰），保证 8 角色色互不撞色且深浅主题自适应；②主角星标★填充从离谱靛 #1e1b4b 改为品牌近黑 #1A1C22、节点名标签从裸灰 #e4e4e7/#a1a1aa 改为 --nv-text-primary/--nv-text-secondary 语义令牌；③游戏模式（game/[nodeId]）新实体发现粒子兜底色从紫 #a78bfa 改为品牌香槟金 #E4B863（canvas 不吃 CSS 变量、用 --nv-gold 真实 hex），与全站统一；④零生产逻辑删除、纯配色收敛、零接口/LLM 变化；⑤质量门禁——tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（与 v3.1.6 同基线、本版零新增测试、纯 UI 配色）；四处版本文件对齐 v3.1.7；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.6 全站离谱色收口 + 界面设计体系文档化（UI 设计师·虚空玻璃体系·收尾）：①离谱色收口——把 v3.1.0 收官后残留的三处紫/粉离谱色（导入向导 ImportWizard 的角色/A路识别色、探索构建 BuildConfigPanel 的 purple/pink 芯片色、设置导入 SettingsImporter 的风格卡计数色）统一收进单一身份色香槟金 --nv-creative，全站配色彻底收敛为「主色/成功/警告/危险/创意金」五语义色，不再有游离的紫粉；②实体高亮测试补锁——给被正文实体高亮重度消费的 src/core/entity-highlighter.ts 补 13 例自动化测试（最长名优先/重叠占用/头边界/停用词过滤/buildEntityMapFromData 别名与覆盖/getCategoryColor 固定色兜底），钉死高亮命中契约；③README 新增「界面与设计体系」章节，把虚空玻璃（无色玻璃身份、非廉价彩色滤纸）、香槟金单一身份色、全站转场（切页轻淡入转屏）、免糊窗毛玻璃、微交互（按钮金环按压/卡片浮起描边发光/错峰入场）体系化说明，让贡献者一眼看懂设计语言；④质量门禁——tsc 0 错误；vitest 全量 115 文件 1190/1190 全绿（较 v3.1.5 基线 115 文件 1177 +13 例）；四处版本文件对齐 v3.1.6；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
  "v3.1.5 世界书词条插入优先级 falsy 陷阱修复（马斯克 CEO 循环运营收口）：①真实缺陷——Agent 工具注册表 tool-registry.ts 的 lore_create 用 insertionOrder = (args.insertionOrder as number) || 50 兜底，但 schema 明写「插入优先级 0-100，越大越靠前」、0 即合法最低优先级，0 || 50 在 JS 把 0 当 falsy 吞成 50、等于用户想设最低优先级却变中高优先级、世界书词条注入顺序被静默反转；outline_create 的 order = order || 1 同源吞 0（想置顶变第 1）；②修复——两处 || 兜底改 ?? 兜底（仅 undefined/null 才用默认值、合法 0 保留），与 v3.1.4 修的 critical 排序 0||2 同源真实缺陷，第一性原理直接修；③零生产逻辑删除、纯局部语义修正、零接口/LLM 变化；④质量门禁——tsc 0 错误；vitest 全量 115 文件 1177/1177 全绿（与 v3.1.4 基线一致、零回归）；四处版本文件对齐 v3.1.5；个人 IP 仍归瑞宝宝无新 IP/品牌/引流。",
