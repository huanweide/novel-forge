...
export const LATEST_VERSION = "v2.0.2";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "v2.0.2 游戏模式「开始游戏」改为 SSE 流式输出，实时逐字显示、默认开启，长等待不再无反馈。",
  "v2.0.2 游戏正文每轮操作带颜色徽标（战斗/对话/探索…），交互有痕迹；结束导出前新增确认弹窗防误触。",
  "v2.0.2 全书写作节奏修复：后台按主线任务收尾判定，不再按章数强制收尾；无规划章数时永不自动结尾。",
  "v2.0.2 摘要入口整合（顶栏「摘要」迁入摘要大纲面板「当前章摘要」）+ 正文新增无外框内联编辑。",
];
