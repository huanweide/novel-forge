...
export const CHANGELOG_BRIEF = [
  "v3.1.72 大纲搜索（OUTLINE-SEARCH · 长篇高频刚需）：①写作台左栏大纲顶部新增紧凑搜索框（放大镜图标 + 输入框 + 清空按钮），输入即过滤、零延迟反馈；模糊匹配章节标题 / 正文前 200 字符 / 状态文字（搜「草稿」「已确认」等中文也行），命中节点保留祖先链（搜「第 5 章」也要看到它所属的卷不被隐藏），搜索框下方实时显示「匹配 X / Y 个节点（含 Z 个祖先卷/分卷）」。②搜索无结果独立分支：「没有匹配「{关键词}」的章节」+「清空搜索」按钮，与「没有章节」空状态分开，避免用户误以为项目空了。③零数据层改动——纯前端 useState + useMemo 计算，不动 schema / API / 存储；只改 OutlineTree.tsx 单文件；性能可控、useMemo 缓存命中、状态映射表按需定义；无障碍：input 与按钮均有 aria-label。④质量门禁——类型检查 0 错误、vitest 全绿、生产构建通过（待跑确认）。个人 IP 仍归瑞宝宝。",
