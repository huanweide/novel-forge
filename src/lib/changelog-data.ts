...
export const LATEST_VERSION = "v3.1.0";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "v3.1.0 UI 收官·余下阶段落地（UI 设计师·虚空玻璃体系·Stage 5+6）：①探讨（ChatPanel）打磨——步骤导航悬浮微抬 + 激活态靛蓝光晕、AI 头像加金色光环、AI 气泡悬浮描边、发送按钮加图标与金环按压，对话流更跟手；②抽卡（CardBrowser）打磨——卡片头部强调条由系统外紫粉（purple-400）收口为香槟金语义令牌、悬浮描边改金色辉光、抽卡逐张错峰入场（animation-delay 按序），点选采纳更有节奏；③预设（创意工坊）打磨——预设卡网格错峰淡入上浮、卡片悬浮顶部金色扫光条、分类 Tab 激活态光晕 + 悬浮微抬，浏览与套用手感统一；④全站路由转场——新增 src/app/template.tsx 接入「转屏」：每次路由切换整页纯 opacity 淡入（0.34s），不碰 transform/filter/clip-path，避免破坏 explore 的 position:fixed 抽屉与全站 sticky 头部定位（工程取舍：组件级 transform 动效保留在页内安全作用域）；⑤离谱色收口——探讨/抽卡/大纲三处残留的系统外 purple-400（去紫粉收敛遗留）全改 --nv-creative 香槟金，全站色值彻底统一；⑥质量门禁——tsc 0 错误；vitest 全量 111 文件 1111/1111 全绿（与 v3.0.0 同基线、本版零新增测试、纯 UI/CSS + 新增转场文件）；四处版本文件对齐 v3.1.0；个人 IP 仍归瑞宝宝，无新 IP/品牌/引流。",
...
export const VERSIONS: VersionEntry[] = [
  {
    version: "v3.1.0",
    date: "2026-08-16",
    title: "UI 收官·余下阶段（探讨/抽卡/预设函数级 + 全站路由转场）",
    sections: [
      { label: "探讨/抽卡/预设函数级打磨", items: ["探讨 ChatPanel：步骤条悬浮微抬、激活蓝晕、AI 头像金环、气泡悬浮描边、发送按钮图标+金环", "抽卡 CardBrowser：卡头强调条收口香槟金、悬浮金辉、逐张错峰入场", "预设 workshop：卡片错峰淡入、悬浮金色扫光、Tab 激活光晕+微抬"] },
      { label: "全站路由转场", items: ["新增 template.tsx，每次路由切换整页纯 opacity 淡入（0.34s），不破坏 fixed 抽屉/sticky 头部", "组件级 transform 动效保留在页内安全作用域"] },
      { label: "离谱色收口 + 质量门禁", items: ["探讨/抽卡/大纲三处残留 purple-400 改 --nv-creative 香槟金，配色彻底统一", "tsc 0 错误；vitest 111 文件 1111/1111 全绿；四处版本文件对齐 v3.1.0"] },
    ],
  },
  {
    version: "v3.0.0",
