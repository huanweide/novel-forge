...
export const LATEST_VERSION = "v0.30.1";

/** 首页公告弹窗摘要（只列最新版本的关键项） */
export const CHANGELOG_BRIEF = [
  "🪟 写作界面新增「宝宝流记忆召回」面板：写章节/微调/续写完成后，实时列出本轮自动呼应的设定与当前生效人设阶段",
  "🎭 闭环透明化：面板直接展示已求值的人设（如「阶段一：陌生人」），不再只是后台 toast，写作者一眼看清 AI 呼应了什么",
  "🔄 每轮生成自动重置面板，避免上一次记忆残留误导",
  "✅ 复用已验证的 recall 事件数据，tsc + 生产构建 + 页面 200 + SSE 数据双层验证通过",
];
