...
export interface FillAfterWritingInput {
  ...
  /** 确认来源标记（auto-confirm/manual/batch），写入填表行 _src 溯源（Max Loop Round3·P5） */
  source?: string;
}
...
export async function safeFillAfterWriting(input: FillAfterWritingInput): Promise<FillResult> {
  ...
  try {
    ...
    const fillRes = await babyloreFill(projectId, content, { projectLlmConfig, chapterOrder: nodeOrder, source: input.source, nodeId });
    babylore = {
      ok: fillRes.ok,
