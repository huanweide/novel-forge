    ...
    <div className="flex flex-col h-full">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-lg p-5 shadow-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500"
              ...
              <div>
                ...
                <textarea className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 resize-none" rows={3}
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <select className="w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-xs"
                ...
                <div>
                  ...
                  <input type="number" className="w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-xs"
                    value={form.priority} onChange={e => setForm({ ...form, priority: parseInt(e.target.value) || 0 })} />
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-5">
              <Button variant="outline" onClick={() => { setShowForm(false); setEditing(null); }} className="border-zinc-700 text-xs h-7">取消</Button>
...
function RuleRow({ rule, onToggle, onEdit, onDelete }: {
  ...
  return (
    <div className={`group flex items-start gap-2 px-2 py-1.5 rounded-lg border transition-colors ${
      rule.enabled ? "border-zinc-800 bg-zinc-900/50 hover:border-zinc-700" : "border-zinc-800/50 bg-zinc-900/30 opacity-60"
    }`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] px-1 rounded bg-zinc-800 text-zinc-500 shrink-0">{CATEGORY_LABELS[rule.category] || rule.category}</span>
