...
> ⭐ **如果 Novel Forge 帮你省了力气，欢迎点个 Star**：你的 Star 是项目继续迭代的动力，也能让更多写作者发现它。Bug、建议、想加的功能，都欢迎开 Issue 或提 PR。
> **当前版本 v3.1.43（彻底移除数据库依赖·本地 SQLite 零配置·无需 Docker）** · 开箱即带 17 个示范预设（套用即开写，免手填表）· 一键示例小说 · 8 题材开局 · 5 格式一键导出（Markdown / TXT / HTML / EPUB / DOCX，PDF 用浏览器打印另存）· 长篇小说不降智（分层记忆 + AI 远楼层压缩）· 批量写作 · 故事线缝合怪 · 世界卡 15 分类体系 · 故事线进度量化。
...
如果第 3 步已用 `npm run dev:db` 启动过，可跳过本步。本步仅启动开发服务器（不重复建库）：
...
> 🎁 **首次打开「创意工坊」即送**：clone 后首次打开「创意工坊」的「全部」页签、且库内还没有内置预设时，会自动播种 17 个内置示范预设（世界观/角色/文风/规则），点一下就能套用，无需手动导入。
>
...
clone 下来首次打开「创意工坊」会自动播种 **17 个内置示范预设**（世界观/角色/文风/规则），打开即见：
...
> 预设是社区共建的最好素材——你打磨出的好文风 / 世界观 / 角色卡，可以导出 `.preset.json` 分享，或写进 Issue 里，我们很乐意把它变成内置示范预设，让更多人免手填表直接开写。
