...
> **当前版本 v3.1.28（探讨模式不再弹数据库警告·体验修复·场景感知）** · 开箱即带 15 个示范预设（套用即开写，免手填表）· 一键示例小说 · 8 题材开局 · 5 格式一键导出（Markdown / TXT / HTML / EPUB / DOCX，PDF 用浏览器打印另存）· 长篇小说不降智（分层记忆 + AI 远楼层压缩）· 批量写作 · 故事线缝合怪 · 世界卡 15 分类体系 · 故事线进度量化。
...
> 📌 **关于格式数量**：上表前 5 种（TXT / Markdown / HTML / EPUB / Word）是应用内**一键导出**按钮直接生成的文件；PDF 一行指的是用浏览器打开 HTML 导出后「打印 → 另存为 PDF」，并非独立导出按钮，故不计为第 6 种导出格式。
