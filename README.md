...
> **当前版本 v1.6.0** · 开箱即带 16 个示范预设 · 一键示例小说 · 8 题材开局 · 6 格式导出 · 自动填表 · 批量写作 · 故事线缝合怪。
...
| 亮点 | 说明 |
|------|------|
| 🧠 **全自动设定管线** | 生成章节 → 自动填结构化表格 → 自动建角色卡/世界书 → 自动检测角色关系 → 自动回写故事线。设定库跟着正文自己长 |
...
| 🎁 **零门槛开局** | 16 个示范预设、一键示例小说《山海拾遗》、8 题材开局，clone 即用 |
...
> 🎁 **首次启动即送**：clone 下来的仓库首次启动自动播种 16 个内置示范预设（世界观/角色/文风/规则），打开「创意工坊」即见，点一下就能套用。
>
...
clone 下来就自带 **16 个「trirui推荐」内置示范预设**，首次启动自动播种，打开「创意工坊」即见：
...
- 📚 **世界书**：设定词条，触发词命中自动注入上下文；AI 自动补充
- 📝 **大纲/章节**：章纲默认收起不挡阅读；选中章节直接「生成/重写」一键直出正文，章名自动生成
- 🧵 **故事线**：主线/支线七要素（欲望→阻碍→行动→结果→意外→转折→结局）；每章进展自动回写；一键打勾完结；**全屏总览弹窗**；缝合怪推进（主线完结自动开新线）
