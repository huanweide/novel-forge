...
> **当前版本 v0.46.6** · 开箱即带 16 个示范预设 · 一键示例小说 · 8 题材开局 · 6 格式导出 · 统计面板 · 工具箱 · 冲突推演。
...
导出弹窗还支持：选择导出的章节范围（整本 / 指定卷章）、是否附带大纲、填写作者署名（会写入 DOCX 页脚与 EPUB 元数据，方便投稿时标注著作权）。

## 写作辅助工具（v0.46.6）

v0.46.6 在工作台内补齐了「成品感」相关的辅助能力，全部本地运行、零配置：

- **新手引导**：首次进入任意工作台会自动弹出三步引导（建好首项目 → 配置 LLM → 开始写作），用 localStorage 记忆，关掉后不再重复打扰。
- **统计面板**（右栏「监测」区）：展示今日写作字数、全书进度、近 7 天写作趋势柱状图、每日目标完成环。数据由 `/api/stats/monitor` 实时统计，无需任何配置。
- **工具箱**（工作台顶部「工具箱」按钮）：把分散的入口收拢成三分类卡片网格（写作辅助 / 内容生成 / 智能分析），一键直达导出、统计、角色卡、世界观等 10 项常用能力。
- **冲突推演**（工具箱内「冲突推演」）：告诉 AI 当前的剧情局势，一次返回 ≥3 个剧情冲突发展选项（含触发点 / 张力 / 走向 / 风险伏笔），每张卡片可一键复制；结果明确标注「仅供参考，最终情节决定权在作者」。
