# Round 1 整合 · 马斯克确认流程（Chair / tri）

> 客观理性中间人整合。七份人格报告 + 三份观测报告已全部落盘。本文件不压缩各角色思想，剖析共识与张力，并据偏差观测的纠偏指令（M1–M6）收敛出**单一权威 UI 规格** `musk-confirm-spec.md`。

---

## 一、会议是否开成了会议

进度观测：七份全部实质长文（600+ 字，均引真实代码），同题收敛，无卡死。质量观测：7/7 达标，5 优 2 良。偏差观测：全部围绕「确认流程按钮 UI」，但共同犯了两个错——①把"马斯克确认"写成"作者确认"（主语漂移）；②各给一套不兼容规格（状态机 7 套、按钮命名 5 种、布局互相打架）。
...
- **T2 状态机**：PG 2 态 vs 马斯克 3 态 vs 芒格 4–5 态。中间态"待确认"要不要？
- **T3 确认时序**：填表在"确认后"跑（马斯克/张雪峰/芒格，堵记忆污染）vs"待确认阶段即跑"（工坊，避免延迟召回）。
- **T4 主语**：6/7 份写成"作者确认"，偏离"马斯克智能体确认"这一用户真实约束。

---

## 四、Chair 纠偏与归一（据 M1–M6）

偏差观测的六条纠偏指令全部采纳，收敛为单一规格 `musk-confirm-spec.md`。核心决策：

- **主语锚定马斯克智能体**（M1）：所有文案/状态/操作者字段以"马斯克智能体"为主语，因为它要真实走通验证。
- **唯一按钮清单 4 键**（M2）：`提交确认` / `打回重写（须填理由）` / `AI诊断` / `批量确认（带护栏）`。否决 PG 无中间态弱化方案。
- **唯一状态机 5 态**（M3）：`outline_only → drafting → pending_confirm → confirmed → project_confirmed`。`drafting` 即"已生成未提交"；`pending_confirm` 即"待马斯克确认"；`confirmed` 即"定稿"；`project_confirmed` 即"整本交付"。打回 `pending_confirm → drafting` 留快照；已确认可 `confirmed → drafting` 重开。
- **唯一布局**（M4）：确认条固定 PostGenPanel 底部常驻栏 + 左栏状态徽标（灰/橙/绿）+ 右栏监测 tab「确认看板」。
- **显式计划流程**（M5）：单列端到端顺序，标注**自动填表必须推迟到 `confirmed` 之后**（采纳马斯克时序根因修复，这是最高杠杆改动）。
...
4. 批量确认护栏阈值（低 qualityScore 拦截线）取多少——待马斯克智能体实测定。

---

*整合：Chair(tri)。七份人格报告见同目录 `jobs/musk/pg/zhangxuefeng/munger/feynman/gongfang.md`；三观测见 `observer-progress/quality/deviation.md`。单一规格见 `musk-confirm-spec.md`。*
