...
### BE-1 正文版本历史与一键回滚 ⭐ 高分功能
- **现在**：`StoryNode.content`（章节正文）只有 `revisionCount` 计数和 `reviewLogs`，**不保留历史版本**；AI 重写/润色一旦覆盖，旧稿永久消失。
- **做完**：每次 AI 写/重写/润色前，把上一版 content 快照存入 `StoryNodeRevision`（或 `reviewLogs` 之外的独立版本表），正文区提供"历史版本"抽屉，可预览并回滚到任意一版。
- **价值**：这是写作者最怕的事——"AI 把我写好的那段改没了"。有了版本历史，AI 再怎么改你都有退路，敢放心让 AI 大改。
- **量级**：中（新增表 + 写入点 + 回滚 UI）。

### BE-2 软删除 + 回收站 ⭐ 高分功能
- **现在**：Project 及所有子表都是 `onDelete: Cascade` **硬删除**；删项目 = 物理删除全部章节/角色/世界书，不可恢复。
- **做完**：项目层加 `deletedAt`（软删除），删除进"回收站"保留 N 天可恢复；子表沿用级联但随项目软删一起隐藏。提供回收站页面。
...
### FE-N1 全局命令面板 + 全文检索（Cmd/Ctrl+K）⭐
- **现在**：工作台里跳到某章、找某个角色、搜一段设定，只能手动在左侧树里翻；**全站没有任何全局搜索**。
- **做完**：加 Cmd/Ctrl+K 命令面板：输入即搜章节标题/角色名/世界书词条/规则，回车跳转；同时支持"新建章节 / 打开设置 / 切换项目"等动作。
- **价值**：项目一大（上百章、几十角色），再也不用滚轮翻树——这是专业写作工具（Obsidian / 飞书）的标配，一上就显"专业"。
- **量级**：中（检索可先走前端内存索引，不必上 ES）。

### FE-N2 项目备份包导出 / 导入（.nfproject）⭐
...
### FE-1 视觉一致性收口：硬编码色值 → --nv-* 令牌 ⭐ 最高优先观感项
...
### FE-9 引入轻量服务端状态层（React Query / SWR）⭐
