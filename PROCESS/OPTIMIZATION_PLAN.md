...
### ✅ FE-N6 时间线视图（与大纲树互补）（v0.46.37 已完成）
- **现在**：故事结构只有"大纲树 + 故事线"，按**叙事顺序**组织；没有按**时间先后**排的视图。
- **做完**：左侧大纲根视图从「分卷/平铺」二态升级为「分卷/平铺/时间线」三态；`StoryNode` 新增 `worldTime String?`，节点上标书中世界时间；时间线视图过滤非卷节点、按 `worldTime` 字符串序排列（未标记排末尾），点击即选中；`CenterPanel` 控制栏新增「世界时间」输入框（失焦回写）。拖拽调整未做（纯文本序排已满足主线需求）。
...
### ✅ FE-N8 自动保存冲突解决（AI 落库 vs 手动编辑）（v0.46.36 已完成）
- **状态**：`StoryNode` 加 `editVersion` 乐观锁字段；`PUT /api/story/nodes/[id]` 支持 `expectedVersion` 条件更新，冲突返回 409 + 库里快照；新建 `SaveConflictModal` 三选项解决（用我的 / 用库里的 / 保留双方）；前端 `handleSaveNode`/`handleDrawSelect`/`onEditOutline` 三处保存携带版本戳、成功回写、409 转交面板。
- **现在（改前）**：大纲是可编辑 textarea，AI 流也会改写 outline；两边并发可能互相覆盖，且无声提示。
...
## 八、进入小说界面 UI 按钮审计（用户第二项指令，#217 系列，2026-08-03 起）

背景：进入小说界面（workspace）顶部栏与次级按钮行累积了大量早期功能按钮，部分数据源错位、互相重复、与创意工坊不联动。用户要求逐项审计：①作用 ②现在还有没有用 ③能否优化成最好版本 ④能否整合统一、与现有功能联动；跟不上的删除或整合。

### ✅ #239 UI审计·文风机制整合（v0.46.38 已完成）
- 问题：顶部栏并排两套文风控件——基于 `styleCard` 的错位标签（`/api/projects/[id]/style` GET 不返回 `styleDescription`，标签永远退化成空按钮）与只读硬编码 `STYLE_TEMPLATES` 的 `StyleSelector` 下拉；二者数据源错位且冗余。且 `page.tsx` 加载时未水合 `styleTemplateId`，激活风格加载后不显示。
- 做法：统一为单一「文风」入口按钮（实时显示激活风格，点击打开 `StyleEditor` 统一风格中枢）；删除冗余 `StyleSelector` 头部下拉；`StyleEditor` 新增「工坊文风」Tab，`GET /api/presets?type=style` 拉取公开预设，「套用」把 `styleDescription` 并入风格笔记、`povType`/`dialogueRatio`/`descriptionRatio` 同步进配置并 `apply` 路由更新文风卡——文风与创意工坊真正联动。修复 `styleTemplateId` 水合。tsc 零错误。

### ✅ #238 UI审计·设定/导入入口厘清（v0.46.39 已完成）
- 问题：「设定」(`SettingsImporter`：粘贴设定文本→AI 拆角色卡+世界书+风格卡，**不建章节**) 与「导入」(`ImportWizard`：粘贴整本书稿→自动分章+抽卡) 在"从文本抽取设定"看似重叠，但主输出完全不同（建章节树 vs 不建）。
- 做法：经评估强行合并会丢能力，故选择**厘清而非硬并**——顶部栏标签改为「导入设定」/「导入书稿」并各自补 tooltip 说明主职责差异（前者拆三卡不建章节，后者整本书稿分章+抽卡）。符合审计「跟不上才删、有用则整合」原则。tsc 零错误。

### ✅ #240 UI审计·次级按钮行去重（v0.46.39 已完成）
- 问题：次级按钮行（结构化表格/创意工坊/项目设定/记忆衰减/项目配置）与「工具箱」对话框肉眼重复——次级行"创意工坊"(`router.push("/workshop")`) 与工具箱入口指向同一页、"结构化表格"(`router.push(/workspace/{id}/tables)`) 同样重复。
- 做法：移除次级行与工具箱完全重复的「结构化表格」「创意工坊」两按钮（留工具箱内入口即可）；保留「项目设定」(`BuildConfigDialog`：小说骨架)、「记忆衰减」、「项目配置」(`ProjectConfigPanel`：书名/模型/LLM参数) 并补 tooltip 区分——确认「项目设定」与「项目配置」职责不同（骨架 vs 书名/模型），非重复，仅补说明。tsc 零错误。

### ✅ #241 UI审计·三卡机制 + 导入大纲（纯审计，无代码改动，2026-08-03 收口）
- 三卡机制（角色卡/世界卡/风格卡）审计结论：**已是数据骨架，无冗余按钮，且已全量联动创意工坊**。
  - 角色卡：左侧栏「角色」tab → `CharacterList`（含「新建角色」）；`导入设定`(`SettingsImporter`) 的「全部三卡」模式；`导入书稿`(`ImportWizard`) 抽角色卡；创意工坊 `character` 预设经 `POST /api/presets/[id]/apply` 直接建 `CharacterCard`（`apply/route.ts:101`）。
  - 世界卡：左侧栏「世界」tab → `WorldPanel`；`导入设定`「仅世界卡」模式；创意工坊 `worldview`/`lorebook` 预设经 `apply` 建 `LorebookEntry`（`apply/route.ts:73/140`）。
  - 风格卡：顶部栏「文风」按钮 + `StyleEditor`（含 #239 加的「工坊文风」Tab）联动 `style` 预设（`apply/route.ts:49`）。
  - 三卡分界由 `src/core/settings/parser.ts` 的 `THREE_CARD_BOUNDARIES` 作为唯一权威规则源严格约束；`syncGlobalPrompt` 在三卡变更后重编译系统提示词使生成立即生效。
  - 判定：角色卡/世界卡经创意工坊套用已落库生效（后端 apply 全覆盖），无需在 workspace 内再造"工坊角色卡/世界卡"浏览入口——那会与创意工坊页（`/workshop`）重复，反违审计"去重"原则。故**仅风格卡**在 workspace 开了 tab（因风格无独立卡片列表页），角色/世界以创意工坊页为枢纽，设计合理。
- 「导入大纲」审计结论：**不存在独立"导入大纲"按钮，且不应新增**。"大纲"工具栏按钮 = AI 生成大纲（`onGenerateOutline`）；"从文本导入大纲"的能力已被 `导入书稿`（粘贴整本书→建章节树，即大纲结构）与 `导入设定`（抽"总纲"）完全吸收；独立的"导入大纲"按钮会与 `导入书稿` 主输出（建章节树）重复，属冗余。判定：保持现状。
- 剩余顶部栏按钮一并审计（均非冗余、保留）：大纲(AI生成)/摘要(章节摘要)/自动化(自动化设置)/工具箱(功能枢纽)/导出(多格式导出+违禁词预检)/备份包(.nfproject)/复制全文(一键复制 Markdown)。其中「复制全文」与「导出」在"取出正文"功能上轻微重叠，但前者是一键 clipboard 快捷、后者是全功能对话框，保留两者符合常见工具设计，不删。

> **#217 系列（进入小说界面 UI 按钮审计）整体收口**：#239 文风机制整合+工坊联动 ✅、#238 导入入口厘清 ✅、#240 次级行去重 ✅、#241 三卡机制+导入大纲审计 ✅（纯审计、无冗余可删、三卡已全量联动创意工坊）。全部按钮审计完毕：该删的删（StyleSelector 下拉、次级行重复入口、10 个废弃 API 端点）、该厘清的厘清（导入设定/书稿）、该联动的联动（文风↔创意工坊）、该保留的保留（三卡骨架、其余功能按钮）。符合用户「跟不上才删、有用则整合」指令。
