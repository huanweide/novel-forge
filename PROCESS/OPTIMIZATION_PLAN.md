...
### ⏳ #238 UI审计·设定/导入入口合并（待做）
- 问题：「设定」(`SettingsImporter`：批量导入设定——角色卡+世界书+风格卡) 与「导入」(`ImportWizard`：导入书稿+抽角色/世界观/文风) 在"从文本抽取设定"高度重叠。
- 方向：明确分工或合并为统一导入入口，避免用户困惑。

### ⏳ #240 UI审计·三卡入口与重复按钮清理（待做）
- 问题：次级按钮行（结构化表格/创意工坊/项目设定/记忆衰减/项目配置）与 `Toolbar`/工具箱肉眼重复（如次级行"创意工坊"与工具箱"创意工坊"重复、"结构化表格"重复）；确认"项目设定"(`setShowBuildConfig`) 与"项目配置"(`setShowProjectConfig`) 是否重复。
- 方向：保留有用、删并冗余。
